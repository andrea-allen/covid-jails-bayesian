...
jhu_data = load_jh_data(saveto='./cached_data/JHU_11-09-21.csv')
...
for facility in set(la_county['Name'].values):
    try:
        ...
        axs[0].legend()
        axs[1].scatter(combined_df['Date'], combined_df[combined_df.columns[6]].diff()/county_pop, label=f'{county_name_lower} daily new', s=2)
        axs[1].scatter(combined_df['Date'], combined_df[combined_df.columns[6]].diff(periods=14)/county_pop, label=f'{county_name_lower} 14-day diff', s=2)
